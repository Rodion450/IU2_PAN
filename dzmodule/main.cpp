...
const double MASS0 = 110000.0;         // Масса самолета, кг (110 тонн)
const double S_WING = 201.5;           // Площадь крыла, м²
...
const double H_START = 500.0;          // Начальная высота, м
const double H_FINISH = 8000.0;        // Конечная высота, м
const double V_START_KMH = 330.0;      // Начальная скорость, км/ч
const double V_FINISH_KMH = 900.0;     // Конечная скорость, км/ч
...
void create_gnuplot_scripts(const TrajectoryResult& traj_time, const TrajectoryResult& traj_fuel) {
    ...
    comp_script << "set output 'TU-154_comparison.png'\n\n";
    comp_script << "set title 'TU-154 Trajectory Comparison (Variant 1)'\n";
    ...
    time_script << "set output 'TU-154_min_time.png'\n\n";
    time_script << "set title 'TU-154 - Minimize Time'\n";
    ...
    fuel_script << "set output 'TU-154_min_fuel.png'\n\n";
    fuel_script << "set title 'TU-154 - Minimize Fuel'\n";
    ...
    bat_file << "@echo off\n";
    bat_file << "echo Running GNUPLOT scripts for TU-154...\n";
    bat_file << "echo.\n";
    ...
    bat_file << "echo - TU-154_comparison.png\n";
    bat_file << "echo - TU-154_min_time.png\n";
    bat_file << "echo - TU-154_min_fuel.png\n";
    bat_file << "echo.\n";
    bat_file << "pause\n";
    bat_file.close();
}
...
int main() {
    ...
    cout << "Dvigateli: 3 x HK-8-2 (" << THRUST_PERCENT << "% nominala)\n";
    ...
    if (choice == 1) {
        ...
        gp_script << "set output 'TU-154_single.png'\n\n";
        gp_script << "set title 'TU-154 - Minimize Time'\n";
        ...
        gp_script << "set terminal pngcairo size 800,600 enhanced\n";
        gp_script << "set output 'TU-154_single.png'\n\n";
        gp_script << "set title 'TU-154 - Minimize Fuel'\n";
        gp_script << "set xlabel 'Speed V (km/h)'\n";
        gp_script << "set ylabel 'Altitude H (m)'\n";
        gp_script << "set grid\n";
        gp_script << "set datafile separator ','\n";
